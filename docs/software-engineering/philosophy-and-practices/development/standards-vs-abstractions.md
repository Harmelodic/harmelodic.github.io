...
Trade-offs are based on what we value. As covered in [Values](../values-and-principles.md#values), we care about aspects
of both the Software and Software Engineering. Standards vs Abstractions ultimate goal is to make _Software_ cheaper and
_Software Engineering_ easier, but most of other trade-offs are ones that touch upon Software Engineering values (some
of which are often highly valued by software engineers but under-considered by managers).
